...
export class DynamoService implements OnModuleInit {
  ...
  constructor() {
    ...
    this.rawClient = new DynamoDBClient(
      this.isLocal
        ? { endpoint: process.env.DYNAMODB_ENDPOINT ?? 'http://localhost:8001', region: 'us-east-1', credentials: { accessKeyId: process.env.AWS_ACCESS_KEY_ID ?? 'local', secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY ?? 'local' } }
