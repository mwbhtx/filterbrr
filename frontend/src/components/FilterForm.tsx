...
export default function FilterForm({
  ...
  const inputCls =
    "w-full rounded bg-input border border-border px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:opacity-50 disabled:cursor-not-allowed";
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      {filter?._source === "temp" && (
        <div className="mx-4 mt-4 px-3 py-2 rounded bg-accent/20 border border-accent/50 text-accent-foreground text-sm">
      ...
      <div className="p-4 space-y-5">
                  ...
                  {sortedGroups.slice(0, sliderValue).map((g) => (
                    <span
                      ...
                      className={`text-[10px] px-1.5 py-0.5 rounded ${
                        g.tier === "high" ? "bg-accent/20 text-accent-foreground" :
                        g.tier === "medium" ? "bg-accent/20 text-accent-foreground" :
                        g.tier === "low" ? "bg-destructive/20 text-destructive" :
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-border">
          {onPromote && (
            <button
              onClick={onPromote}
              className="px-4 py-2 text-sm font-medium rounded bg-accent hover:bg-accent/90 text-accent-foreground transition-colors"
            >
              Save Filter
            </button>
          )}
          {!readOnly && !onPromote && (
            <button
              onClick={handleSave}
              className="px-4 py-2 text-sm font-medium rounded bg-primary hover:bg-primary/90 text-primary-foreground transition-colors"
            >
              Save
            </button>
          )}
          {onCancel && isDirty && (
            <button
              onClick={onCancel}
              className="px-4 py-2 text-sm font-medium rounded bg-muted hover:bg-muted text-muted-foreground border border-border transition-colors"
            >
              Cancel
            </button>
          )}
          ...
          {onDelete && !readOnly && (
            <button
              onClick={onDelete}
              className="px-4 py-2 text-sm font-medium rounded bg-destructive/20 hover:bg-destructive/80 text-destructive transition-colors ml-auto"
            >
              Delete
            </button>
          )}
